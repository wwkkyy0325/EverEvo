...
type SandboxState struct {
	Active  sandbox.Name `json:"active"`  // currently "alpha" or "beta"
	Pending sandbox.Name `json:"pending"` // the one being verified
}
...
func CurrentSandbox(projectRoot string) sandbox.Name {
...
func SaveSandboxState(projectRoot string, active sandbox.Name) error {
	dir := filepath.Join(projectRoot, "data")
	os.MkdirAll(dir, 0755)
	return os.WriteFile(filepath.Join(dir, ".sandbox_state"), []byte(active), 0644)
}

// BuildPath returns the default wails build output path.
func BuildPath(projectRoot string) string {
	return filepath.Join(projectRoot, "build", "bin", "everevo.exe")
}
...
func RunEvolutionCycle(projectRoot, buildExePath string) (*sandbox.Instance, error) {
	// Determine target sandbox.
	current := CurrentSandbox(projectRoot)
	var target sandbox.Name
	if current == "" || current == sandbox.Beta {
		target = sandbox.Alpha
	} else {
		target = sandbox.Beta
	}
	...
	if inst, err := sandbox.Prepare(projectRoot, target, buildExePath); err == nil {
		_ = inst.Stop() // best-effort cleanup
	}

	// Prepare the sandbox with the new EXE.
	inst, err := sandbox.Prepare(projectRoot, target, buildExePath)
	if err != nil {
...
func AcceptEvolution(projectRoot string, inst *sandbox.Instance) error {
...
func RejectEvolution(inst *sandbox.Instance, reason string) error {
