...
// For external MCP tools it prefers the raw inputSchema to avoid JSON Schema
// fidelity loss; for internal tools it uses the typed Parameters.
func marshalAgentToolDefs(defs []*tools.ToolDef) json.RawMessage {
	...
	for _, d := range defs {
		...
		if len(d.RawParameters) > 0 {
			t.Function.Parameters = d.RawParameters
