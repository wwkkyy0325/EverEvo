...
func (p *Plugin) ToolDefs() []core.ToolDef {
	return []core.ToolDef{
		...
		{
			Name: "catalog_set_token", Description: "设置平台 API Token 以提升下载速度和访问权限。",
			ReadOnly: false,
			Parameters: map[string]any{
				"type": "object",
				"properties": map[string]any{
		...
		{
			Name: "catalog_get_accounts", Description: "获取各平台账号登录状态。",
			ReadOnly: true,
			Parameters: map[string]any{
				"type": "object", "properties": map[string]any{},
			},
		...
		// ── toolbox ──
		{
			Name: "toolbox_list_models", Description: "列出工具箱可用的模型（句向量/图像分类等）。",
			ReadOnly: true,
			Parameters: map[string]any{
				"type": "object", "properties": map[string]any{},
			},
		...
		{
			Name: "toolbox_embed", Description: "使用句向量模型将文本编码为嵌入向量。",
			ReadOnly: false,
			Parameters: map[string]any{
				"type": "object",
				"properties": map[string]any{
...
func (p *Plugin) CallTool(_ context.Context, name string, args map[string]any) (core.ToolResult, error) {
	...
	case "catalog_set_token":
		src, tok := getStr(args, "source"), getStr(args, "token")
		if src == "" {
			return errMsg("缺少必填参数: source"), nil
		}
		if e := SetAccountToken(p.backend, src, tok); e != nil {
			return err(e), nil
		}
		return ok(map[string]string{"status": "ok"}), nil
	case "catalog_get_accounts":
		return ok(GetAccounts(p.backend)), nil
