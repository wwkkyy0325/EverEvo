...
// Falls back to paragraph chunking if no clear boundaries.
func chunkCode(text string) []string {
	// Find lines that start a function/method/class definition.
	...
	// Group lines between boundaries, merge small groups.
	var chunks []string
	var current []string
	for i := 0; i < len(boundaries); i++ {
		...
		if len([]rune(block)) > 3000 {
			// Oversized block — sub-chunk with paragraph splitter.
	...
	// If chunkCode produced nothing useful (e.g. no function defs found), fall back.
	if len(chunks) == 0 {
		return rag.ChunkText(text)
	}
	return chunks
...
func chunkConfig(text string) []string {
	// Config files are usually small and losing structure hurts retrieval.
	if len([]rune(text)) <= 2000 {
		return []string{text}
	}
	// Large JSON/YAML — split by top-level key sections.
	return rag.ChunkText(text)
}
