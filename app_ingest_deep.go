...
func (a *App) IngestDeepAnalyze(dirPath string) (*DeepAnalysis, error) {
...
func (a *App) IngestDeepCommit(analysis *DeepAnalysis) (*IngestResult, error) {
	...
	for di, domain := range analysis.Domains {
		...
		log.Printf("[deep] 域 %s: %d chunks", domain.Name, domainChunks)
		...
		agent := agents.Agent{
			Name: sanitizeAgentName(domain.Name) + " 专家",
...
func (a *App) IngestDeepReview(analysis *DeepAnalysis) (map[string]any, error) {
	provider, err := a.resolveActiveProvider()
	if err != nil {
		return nil, err
	}
...
func (a *App) IngestDeep(dirPath string) (*DeepAnalysis, error) {
	analysis, err := a.IngestDeepAnalyze(dirPath)
	if err != nil {
		return nil, err
	}
	if _, err := a.IngestDeepCommit(analysis); err != nil {
		return nil, err
	}
	// Phase 4 runs async — don't block the user.
	go func() {
		if _, err := a.IngestDeepReview(analysis); err != nil {
